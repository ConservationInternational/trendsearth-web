...
class Api(object):
    ...
    def recover_pwd(self, email):
        return self.call_api(u'/api/v1/user/{}/recover-password'.format(quote_plus(email)), 'post')
    ...
    def update_password(self, password, repeatPassword):
        ...
        return self.call_api(u'/api/v1/user/me', 'patch', payload, use_token=True)
    ...
    def get_execution(self, id=None, date=None):
        ...
        if not resp:
            ...
            data = sorted(data, key=lambda job_dict: round(
                datetime.strptime(job_dict['start_date'],
