...
/***************************************************************************
 LDMP - A QGIS plugin
 This plugin supports monitoring and reporting of land degradation to the
 UNCCD and in support of the SDG Land Degradation Neutrality (LDN) target.
                              -------------------
        begin                : 2017-05-23
        git sha              : $Format:%H$
...
def read_json(f, verify=True):
    ...
    if not os.path.exists(filename):
        ...
        try:
            ...
            # QtWidgets.QMessageBox.critical(None,
            # tr_download.tr("Error"),
            # tr_download.tr("""Unable to write to {}.
            # Do you need administrator permissions?""".format(filename)))
            return None
        resp = worker.get_resp()
        if not resp:
            return None
...
class Download(object):
    ...
    def start(self):
        try:
            ...
            log("""Download failed due to ChunkedEncodingError -
             likely a connection loss""")
