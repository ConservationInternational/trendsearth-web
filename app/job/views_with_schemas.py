...
def process_drought_vulnerability_with_schema(request):
    ...
    try:
        # Validate parameters using schema
        validated_data = validate_and_transform_parameters('drought-vulnerability', request)[0]
...
def ajax_run_job_with_schemas(request):
    ...
    try:
        ...
        if not schema:
            return JsonResponse({'error': f'No schema available for algorithm: {algo_name}'}, status=400)
...
def get_schema_info(request):
    ...
    try:
        ...
        if not fields:
            return JsonResponse({'error': f'No schema found for script: {script_name}'}, status=404)
