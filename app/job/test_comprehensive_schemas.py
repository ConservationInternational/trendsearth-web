...
def test_all_schema_validations():
    ...
    try:
        ...
        for script_name, test_data in test_cases.items():
            ...
            try:
                validated = schema_registry.validate_parameters(script_name, test_data['valid'])
...
def test_schema_metadata():
    ...
    try:
        ...
        if metadata_found == len(available_schemas):
            ...
            print(f"✗ Only {metadata_found}/{len(available_schemas)} schemas have descriptions")
            return False
...
def test_edge_cases():
    ...
    try:
        ...
        boundary_tests = [
            ('drought-vulnerability', {'aoi_id': 1, 'task_name': 'Test', 'year_initial': 2010, 'year_final': 2020, 'lag_cb': 1}, True),  # Min valid
            ('drought-vulnerability', {'aoi_id': 1, 'task_name': 'Test', 'year_initial': 2010, 'year_final': 2020, 'lag_cb': 12}, True), # Max valid
            ('drought-vulnerability', {'aoi_id': 1, 'task_name': 'Test', 'year_initial': 2010, 'year_final': 2020, 'lag_cb': 0}, False), # Below min
            ('drought-vulnerability', {'aoi_id': 1, 'task_name': 'Test', 'year_initial': 2010, 'year_final': 2020, 'lag_cb': 13}, False), # Above max
        ]
        ...
        if boundary_passed == len(boundary_tests):
            ...
            print(f"✗ Only {boundary_passed}/{len(boundary_tests)} boundary tests passed")
            return False
