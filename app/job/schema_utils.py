...
def validate_job_parameters(
    script_name: str, request_data: Dict[str, Any]
) -> Dict[str, Any]:
    """
...
def populate_aoi_data(validated_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        validated_data["crosses_180th"] = (
            False  # Could be calculated using te_schemas if needed
        )

        logger.debug(
            f"Populated AOI data for aoi_id: {aoi_id} using te_schemas.aoi.AOI"
        )
        return validated_data
...
def get_schema_fields_for_gui(script_name: str) -> Optional[Dict[str, Any]]:
    ...
    for field_name, field_info in fields.items():
        ...
        if "validators" in field_info:
            gui_field["validation"] = _extract_validation_constraints(
...
def validate_and_transform_parameters(
    script_name: str, request
) -> List[Dict[str, Any]]:
    """
