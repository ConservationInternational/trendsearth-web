...
class SoilCarbonSchema(BaseJobSchema, DateRangeSchema):
    ...
    soc_method = fields.Str(
        ...
        metadata={'title': 'SOC calculation method', 'description': 'Method for calculating soil organic carbon change'}
    )
    
    # Climate regime for SOC calculations
    climate_regime = fields.Str(
        validate=validate.OneOf(['temperate-dry', 'temperate-moist', 'tropical-dry', 'tropical-moist']),
        metadata={'title': 'Climate regime', 'description': 'Climate regime for SOC calculations'}
    )
