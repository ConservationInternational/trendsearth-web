...
class SubIndicatorsSchema(BaseJobSchema, DateRangeSchema):
    ...
    calculate_productivity = fields.Bool(
        ...
        metadata={
            ...
            "description": "Include land productivity sub-indicator",
        },
    )
    ...
    calculate_land_cover = fields.Bool(
        load_default=True,
        metadata={
            "title": "Calculate land cover",
            "description": "Include land cover change sub-indicator",
        },
    )
    ...
    calculate_soil_carbon = fields.Bool(
        load_default=True,
        metadata={
            "title": "Calculate soil carbon",
            "description": "Include soil organic carbon sub-indicator",
        },
    )
        ...
        if data.get("calculate_productivity", True):
            if not data.get("ndvi_dataset"):
                raise ValidationError(
                    "ndvi_dataset is required when calculate_productivity is enabled"
                )
    ...
    def process_transition_matrix(self, data, **kwargs):
        ...
        if "_validated_matrix" in data:
            ...
            data["trans_matrix"] = LCTransitionDefinitionDeg.Schema().dump(
                data["_validated_matrix"]
            )
    ...
    class Meta:
        ...
        description = (
            "SDG 15.3.1 sub-indicators analysis parameters using te_schemas validation"
        )
