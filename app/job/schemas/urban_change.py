...
class UrbanChangeSchema(BaseJobSchema):
    ...
    un_adju = fields.Float(
        ...
        metadata={'title': 'UN adjustment factor', 'description': 'Urban area adjustment factor'}
    )
    
    isi_thr = fields.Float(
        required=True,
        validate=validate.Range(min=0.0, max=1.0),
        metadata={'title': 'ISI threshold', 'description': 'Impervious Surface Index threshold'}
    )
    
    ntl_thr = fields.Float(
        required=True,
        validate=validate.Range(min=0.0, max=100.0),
        metadata={'title': 'NTL threshold', 'description': 'Night Time Lights threshold'}
    )
    ...
    cap_ope = fields.Float(
        required=True,
        validate=validate.Range(min=0.0, max=1.0),
        metadata={'title': 'Capture open areas', 'description': 'Factor for capturing open urban areas'}
    )
    
    pct_suburban = fields.Float(
        required=True,
        validate=validate.Range(min=0.0, max=100.0),
        metadata={'title': 'Suburban percentage', 'description': 'Percentage threshold for suburban areas'}
    )
    
    pct_urban = fields.Float(
        required=True,
        validate=validate.Range(min=0.0, max=100.0),
        metadata={'title': 'Urban percentage', 'description': 'Percentage threshold for urban areas'}
    )
